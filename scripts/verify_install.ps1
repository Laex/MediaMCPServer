...
Add-Check 'MCP config' (Test-Path (Join-Path $CursorDir 'mcp.json')) '.cursor\mcp.json' 'info'
Add-Check 'src\MediaMCPServer.dpr' (Test-Path (Join-Path $SrcDir 'MediaMCPServer.dpr')) 'Delphi project' 'info'

foreach ($sub in @('captures', 'output', 'faces', 'video')) {
    Add-Check "data\media\$sub\" (Test-Path (Join-Path $MediaDir $sub)) '' 'info'
}

Write-Host "=== Media MCP Server - install verification ===" -ForegroundColor Cyan
...
if ($fail -gt 0) { Write-Host 'Install incomplete. Run: .\install.ps1' -ForegroundColor Red; exit 1 }

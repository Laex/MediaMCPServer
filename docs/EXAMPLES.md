...
- **HTTP-режим:** для длительных пайплайнов удобно держать сервер в Streamable HTTP (`.\bin\launch_http.cmd`) и увеличить таймаут инструментов в клиенте (Cursor / Codex: `tool_timeout_sec`).
...
- **Таймауты:** RTSP и запись 90 с — долгие операции; в HTTP-режиме увеличьте `tool_timeout_sec` (Cursor / Codex) или держите stdio-сессию.
...
- **Таймауты:** `webcam_record_video` (15 с) и `video_track_object` (60 кадров) — долгие операции; в HTTP-режиме увеличьте `tool_timeout_sec` (Cursor / Codex).
